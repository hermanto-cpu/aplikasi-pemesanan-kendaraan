    ...
    <div class="sidebar border border-right col-md-3 col-lg-2 p-0 bg-body-tertiary">
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        <div class="offcanvas-header">
          <h5 class="offcanvas-title" id="sidebarMenuLabel">Aplikasi Pemesananan Kendaraan</h5>
      ...
      <div class="table-responsive small">
        <table class="table table-striped table-sm">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $submission->destination ?? 'Not Assigned'}}</td>
              @foreach ($drivers as $driver)
              <td>{{ $driver->name ?? 'Not Assigned'}}</td>
              @endforeach
              @foreach ($pemesan as $order)
              <td>{{ $order->name ?? 'Not Assigned' }}</td>
              @endforeach
              @foreach ($supervisors_1 as $supervisor)
              <td>{{ $supervisor->name ?? 'Not Approved' }}</td>
              @endforeach
              @foreach ($supervisors_2 as $supervisor_2)
              <td>{{ $supervisor_2->name ?? 'Not Approved' }}</td>
              @endforeach
                    <td>{{ $submission->kendaraan ?? 'Not Assigned'}}</td>
                    <td>
                      {{-- <form action="/dashboard/action" method="GET">
                      @csrf --}}
                      @if($submission->status === 'pending' && $supervisors_1->contains(auth()->user()->id))
                      <button class="btn btn-success" type="button" onclick="window.location='{{ route('dashboard.action', ['submission_id' => $submission->id, 'action' => 'dashboard/action']) }}'">
                        <i class="bi bi-pencil"></i> Approve
                      </button>
                      @elseif($submission->status === 'approved_level_1' && $supervisors_2->contains(auth()->user()->id))
                      <button class="btn btn-success" type="button" onclick="window.location='{{ route('dashboard.action', ['submission_id' => $submission->id, 'action' => 'dashboard/action']) }}'">
                        <i class="bi bi-pencil"></i> Approve
                      </button>
                      @elseif($submission->status === 'approved_level_2')
                      <button class="btn btn-success">
                        <i class="bi bi-check"></i> Done
                      </button>
                      @else
                      <button class="btn btn-primary" type="button" onclick="window.location='{{ route('dashboard.action', ['submission_id' => $submission->id, 'action' => 'dashboard/action']) }}'">
